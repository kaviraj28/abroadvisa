...
@section('title', 'Edit ' . $roadmap->name . ' - Visa Abroad')
    ...
    <div class="content">
        <div class="card container-fluid mb-4">
            <div class="card-header d-flex justify-content-between align-items-center">
                <h5 class="mb-0">Edit roadmap - {{ $roadmap->name }}</h5>
                <small class="text-muted float-end">
                    <a class="btn btn-primary" href="{{ route('roadmap.index') }}"><i class="fa-solid fa-arrow-left"></i>
                        Back</a>
                </small>
            </div>
            <div class="card-body p-0">
                <form class="row" method="POST" action="{{ route('roadmap.update', $roadmap->id) }}"
                        ...
                        <div class="card card-body main-description shadow br-8 p-4">
                            <div class="form-group mb-3">
                                ...
                                <input class="form-control br-8 @error('name') is-invalid @enderror" type="text"
                                    name="name" value="{{ old('name', $roadmap->name) }}" placeholder="Enter Name">
                                @error('name')
                            ...
                            <div class="form-group mb-3">
                                ...
                                <textarea class="form-control ckeditor br-8 @error('description') is-invalid @enderror" id="description"
                                    name="description" rows="10" placeholder="Enter Description">{{ old('description', $roadmap->description) }}</textarea>
                                @error('description')
                                    <div class="invalid-feedback" style="display: block;">
                                        {{ $message }}
                                    </div>
                        ...
                        <div class="card-body card shadow br-8">
                            <div class="form-group mb-3 d-flex align-items-center">
                                ...
                                <select class="form-select ms-5" id="status" name="status">
                                    <option class="p-3" value="0" @if ($roadmap->status == 0) selected @endif>
                                        Draft</option>
                                    <option class="p-3"@if ($roadmap->status == 1) selected @endif value="1">
                                        Publish</option>
                                </select>
                            </div>
                            ...
                            <div class="form-group mb-3 d-flex align-items-center">
                                ...
                                <input class="form-control ms-5 @error('order') is-invalid @enderror" type="number"
                                    name="order" value="{{ old('order', $roadmap->order) }}" placeholder="Enter Order">
                                @error('order')
                                    <div class="invalid-feedback" style="display: block;">
                                        {{ $message }}
                                    </div>
                                    ...
                                    @if ($category->isNotEmpty())
                                        @foreach ($category as $key => $value)
                                            <option class="p-3" value="{{ $value->id }}"
                                                {{ $value->id == $roadmap->services ? 'selected' : '' }}>
                                                {{ $value->name }}
                                            </option>
                                        @endforeach
                                    @endif
                                    ...
                                    <a class="feature-modal" data-bs-toggle="modal" data-bs-target="#featureModel"
                                        ...
                                        <div
                                            ...
                                            <div
                                                ...
                                                @if ($roadmap->image)
                                                    @php
                                                        $feature = get_media($roadmap->image ?? '');
                                                    @endphp
                                                    @if ($feature)
